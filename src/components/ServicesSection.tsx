    ...
    <section id="services" className="py-12 sm:py-16 lg:py-20 bg-brand-offWhite text-brand-charcoal relative border-y border-gray-200 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-12 sm:mb-16">
          {SERVICES.map((service: ServiceItem, idx: number) => (
            <motion.div 
              ...
              whileHover={{ y: -6 }}
              className="bg-white rounded-[4px] p-6 sm:p-7 border border-gray-200 shadow-sm hover:shadow-2xl hover:border-brand-green/60 transition-all flex flex-col justify-between group"
            >
              <div className="space-y-5">
                <div className="w-14 h-14 rounded-[4px] bg-brand-lightGreen text-brand-green flex items-center justify-center group-hover:bg-brand-green group-hover:text-white transition-colors border border-brand-green/20">
                ...
                <ul className="space-y-2 pt-2 border-t border-gray-100 text-xs text-gray-700">
                  {service.details.map((detail, dIdx) => (
                    <li key={dIdx} className="flex items-start gap-2">
                      <span className="w-1.5 h-1.5 rounded-[2px] bg-brand-green shrink-0 mt-1.5"></span>
                      <span>{detail}</span>
                    </li>
                  ))}
                </ul>
              ...
              <div className="pt-5 mt-5 border-t border-gray-100">
                <button
                  ...
                  className="w-full py-2.5 rounded-[4px] bg-brand-offWhite group-hover:bg-brand-green text-brand-charcoal group-hover:text-white text-xs font-bold transition-all flex items-center justify-center gap-2 border border-gray-200 group-hover:border-transparent active:scale-95"
                >
        ...
        <motion.div 
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="bg-brand-charcoal text-white rounded-[4px] p-6 sm:p-10 lg:p-12 shadow-2xl relative overflow-hidden architectural-grid-dark border border-brand-green/30"
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 relative z-10">
            {WHY_CHOOSE_KKR.map((item, idx) => (
              <motion.div 
                ...
                className="p-5 sm:p-6 rounded-[4px] bg-white/5 border border-white/10 hover:border-brand-green/60 transition-all hover:bg-white/10 group"
              >
                <div className="flex items-start gap-3.5">
                  <div className="p-2.5 rounded-[4px] bg-brand-green/20 text-brand-green group-hover:bg-brand-green group-hover:text-white transition-colors shrink-0">
